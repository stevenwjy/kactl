 ...
 * Description: General matching with no failure probability.
 * Intialize in blossom.init(V, edgeLists) and set up M as max Vertex.
 * Output in blossom.edmonds() and match[i] is matched pair of i (-1 if there isn't a matched pair).
 * Time: O(N^3)
 */
#pragma once
...
struct Blossom {
    ...
    void init(int total_vertex, vector<pair<int,int>> edgeList) {
        V = total_vertex;
        E = edgeList.size();
        for (auto e: edgeList) {
            int u = e.first;
